...
/**
 * Update a single corpus in the chart data
 * @param {Object} currentChartData - Current chart data
 * @param {number} corpusIndex - Index of corpus to update
 * @param {Object} newCorpusData - New data for the corpus
 * @returns {Object} Updated chart data
 */
export const updateCorpusInChartData = (currentChartData, corpusIndex, newCorpusData) => {
    if (!currentChartData || corpusIndex < 0 || corpusIndex >= currentChartData.rawData.length) {
        return currentChartData;
    }
    
    const newRawData = [...currentChartData.rawData];
    const updatedCorpus = {
        ...newRawData[corpusIndex],
        ...newCorpusData,
        frequency: calculateFrequency(newCorpusData.matches || 0, newCorpusData.wordsScanned)
    };
    
    newRawData[corpusIndex] = updatedCorpus;
    
    const newFrequencies = [...currentChartData.frequencies];
    newFrequencies[corpusIndex] = updatedCorpus.frequency;
